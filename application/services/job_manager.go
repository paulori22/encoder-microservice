...
import (
	...
	"github.com/paulori22/encoder-microservice/framework/queue"
...
func (j *JobManager) Start(ch *amqp.Channel) {
	...
	if err != nil {
		log.Fatalf("error loading env var CONCURRENCY_WORKERS")
	}
...
func (j *JobManager) checkParseErrors(jobResult JobWorkerResult) error {
	if jobResult.Job.ID != "" {
		log.Printf("MessageID #{jobResult.Message.DeleveryTag}. Error parsing job: #{jobResult.Job.ID}")
	} else {
		log.Printf("MessageID #{jobResult.Message.DeleveryTag}. Error parsing message: #{jobResult.Error}")
	}
